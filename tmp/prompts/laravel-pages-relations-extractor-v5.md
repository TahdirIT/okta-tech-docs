...
3) Add child -> hub relations using:
   relation: parent-child
4) Do NOT add child -> child relations unless there is explicit page-local evidence (link/form/redirect/action) in the child page itself.
5) Keep auth edges for protected pages when applicable (example: page -> login as auth).
6) When both parent-child and navigation could apply, prefer parent-child for structural module edges.
...
1) auth         (login/logout/context completion/guard-driven flow)
2) form         (submit/action result)
3) parent-child (module hierarchy edge inferred from route/module structure)
4) navigation   (explicit user page transition from page-local UI)
